...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier verifyCaller(address _address) {
    require(msg.sender == _address, "msg.sender is not the right Caller");
    _;
  }
  ...
  modifier checkValue(uint256 _upc) {
    ...
    uint256 amountToReturn = msg.value.sub(_price);
  ...
  modifier processed(uint256 _upc) {
    require(items[_upc].itemState == State.Processed, "This Item is not processed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint256 _upc) {
    require(items[_upc].itemState == State.Packed, "This Item is not packet");
    _;
  }
  ...
  modifier sold(uint256 _upc) {
    require(items[_upc].itemState == State.Sold, "This Item is not Sold");
    _;
  }
 
  // Define a modifier that checks if an item.state of a upc is Shipped
  modifier shipped(uint256 _upc) {
    require(items[_upc].itemState == State.Shipped, "This Item is not ");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint256 _upc) {
    require(items[_upc].itemState == State.Received, "This Item is not Received");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint256 _upc) {
    require(items[_upc].itemState == State.Purchased, "This Item is not Purchased");
    _;
  }
  ...
  function harvestItem(uint256 _upc,
    ...
    public
    // Checks msg.sender is farmer
    onlyFarmer()
  {

    address ceroAddress = 0x0000000000000000000000000000000000000000;
    Item memory item;
    ...
    item.productID = sku.add(_upc);
    item.productNotes = _productNotes;
    item.productPrice = 0;
    item.itemState = State.Harvested;
    item.distributorID = ceroAddress;
    item.retailerID = ceroAddress;
    item.consumerID = address(uint160(ceroAddress));  //Conversion a payable
  ...
  function buyItem(uint256 _upc) public payable
    // Checks msg.sender is Distributor
    onlyDistributor()
    // Call modifier to check if upc has passed previous supply chain stage
    forSale(_upc)
